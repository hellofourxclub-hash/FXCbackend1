...
router.post('/courses/:courseId/plan', authMiddleware, async (req, res) => {
  try {
    ...
    const samePlan = course.razorpayPlanId &&
      Number(course.razorpayPlanAmount) === amountPaise &&
      course.razorpayPlanPeriod === period &&
      Number(course.razorpayPlanInterval) === interval;

    if (samePlan) {
      return res.json({ planId: course.razorpayPlanId, courseId: String(course._id), amount, currency: 'INR', period, interval, reused: true });
    }

    // Razorpay plans are immutable. A changed price or billing interval gets a new plan;
    // existing subscribers remain attached to their original plan.
    const plan = await razorpay.plans.create({
      period,
      interval,
      item: { name: course.title, amount: amountPaise, currency: 'INR', description: `${course.title} recurring subscription` },
...
router.post('/create', async (req, res) => {
  try {
    ...
    await Subscription.create({
      ...
      customerName: String(name).trim().slice(0, 120),
