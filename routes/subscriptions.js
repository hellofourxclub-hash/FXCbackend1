...
const toRazorpayPeriod = (period) => PERIOD_MAP[period] || null;

// Admin-only: create/recreate the Razorpay plan for a course's current billing configuration.
// A Razorpay plan is immutable; changing the course billing configuration creates a new plan.
router.post('/courses/:courseId/plan', authMiddleware, async (req, res) => {
  try {
    ...
    const period = toRazorpayPeriod(course.billingPeriod);
    ...
    const plan = await razorpay.plans.create({
      ...
      item: {
        ...
        currency: 'INR',
        description: `${course.title} recurring subscription`,
      },
...
// Customer-facing: create a subscription from the course's current Razorpay plan.
router.post('/create', async (req, res) => {
  try {
    const { courseId, name, email, phone } = req.body || {};
    if (!courseId || !name || !email) {
      return res.status(400).json({ message: 'courseId, name and email are required' });
    }

    const course = await Course.findById(courseId).lean();
    if (!course || !course.isActive) return res.status(404).json({ message: 'Course not found or inactive' });
    if (!course.billingEnabled || !course.razorpayPlanId) {
      return res.status(400).json({ message: 'Recurring billing is not configured for this course' });
    }
    ...
    const subscription = await razorpay.subscriptions.create({
      ...
      notes: {
        courseId: String(course._id),
        customerEmail: String(email).slice(0, 254),
      },
    });
    ...
    await Subscription.create({
      ...
      amount: Number(course.discountPrice && course.discountPrice < course.price ? course.discountPrice : course.price),
      currency: 'INR',
      interval: course.billingInterval,
    ...
    res.status(201).json({
      ...
      planId: course.razorpayPlanId,
      amount: Math.round((course.discountPrice && course.discountPrice < course.price ? course.discountPrice : course.price) * 100),
      currency: 'INR',
...
// Must be mounted before express.json(); req.body is the exact raw webhook payload.
