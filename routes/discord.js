...
// Customer-facing: generate a short-lived OAuth URL tied to a purchase/subscription context.
router.get('/oauth/start', (req, res) => {
  if (!configReady()) return res.status(503).json({ message: 'Discord integration is not configured' });
  const { customerKey } = req.query;
  if (!customerKey || typeof customerKey !== 'string' || customerKey.length > 200) {
...
// OAuth callback: exchange code and bind the authenticated Discord User ID to FXC entitlements.
router.get('/oauth/callback', async (req, res) => {
  if (!configReady()) return res.status(503).send('Discord integration is not configured');

  const { code, state } = req.query;
  const payload = verifyState(state);
  if (!code || typeof code !== 'string' || !payload) {
  ...
  try {
    ...
    if (!entitlements.length) {
      return res.status(403).send('No active FXC entitlement was found for this connection.');
    }

    // Bind the authenticated Discord identity to all active entitlements for the customer.
    await Entitlement.updateMany(
      { customerKey: payload.customerKey, status: 'active' },
      { $set: { discordUserId } }
    );

    res.redirect(`${process.env.FRONTEND_URL || 'https://fourxclub.in'}?discord=connected`);
  } catch (error) {
...
// Admin-only diagnostic endpoint to list the configured guild without exposing bot secrets.
